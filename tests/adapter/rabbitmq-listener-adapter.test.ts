...
describe('RabbitMQListenerAdapter', () => {
  ...
  it('should call assertQueue() for each listener', async () => {
    ...
    expect(assertQueue).toHaveBeenNthCalledWith(1, 'queue1', { durable: true })
    expect(assertQueue).toHaveBeenNthCalledWith(2, 'queue2', { durable: true })
  })

  it('should call consume() for each listener', async () => {
    await sut.listen()

    expect(consume).toHaveBeenNthCalledWith(1, 'queue1', expect.any(Function), { noAck: false })
    expect(consume).toHaveBeenNthCalledWith(2, 'queue2', expect.any(Function), { noAck: false })
  })
  ...
  it('should call ack() for each listener', async () => {
    await sut.listen()
    const message1 = { content: Buffer.from(JSON.stringify({ key: 'value1' })) }
    const message2 = { content: Buffer.from(JSON.stringify({ key: 'value2' })) }
    const consumeCallback1 = consume.mock.calls[0][1]
